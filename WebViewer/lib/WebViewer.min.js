...
if(this.options.autoCreate)this.create()};PDFTron.WebViewer.prototype={version:"3.0.0",create:function(){var me=this;if(typeof this.options.initialDoc!=="undefined"){var docPath=this._correctRelativePath(this.options.initialDoc);docPath=encodeURIComponent(docPath);this.options.initialDoc=docPath;this._create()}else if(typeof this.options.cloudApiId!=="undefined")$.get("https://api.pdftron.com/v2/download/"+this.options.cloudApiId+"?type=xod&redirect=false",function(data){if(typeof data.url==="undefined")me.loadErrorPage();
else{me.options.initialDoc=encodeURIComponent(data.url);me._create()}},"json").error(function(){me.loadErrorPage()});else this._create()},_create:function(){var me=this;me.rcId=((1+Math.random())*65536|0).toString(16).substring(1);if(typeof this._trigger==="undefined")this._trigger=function(type,event,data1,data2){event=$.Event(event);event.type=type;event.target=$(this.element)[0];var data=[];if(typeof data1!=="undefined")data.push(data1);if(typeof data2!=="undefined")data.push(data2);$(this.element).trigger(event,
data)};var viewers=this.options.type.replace(" ","").split(",");if(viewers.length<1)viewers[0]="html5";me._createViewer(viewers)},_notSupportedMobile:function(){$(this.element).append('<div id="webviewer-browser-unsupported">PDF document viewing is not currently supported by the mobile viewer.</div>')},_createViewer:function(viewers){var me=this;me.selectedType="none";if(this.isMobileDevice()){if(this.options.documentType&&this.options.documentType!=="xod"){this._notSupportedMobile();return}viewers=
Array("html5Mobile");me.selectedType="html5Mobile";if(this.options.mobileRedirect){var newLocation=this.options.html5MobilePath+this._getHTML5OptionsURL();window.location=newLocation;return}}var allowHTML5=false;for(var i=0;i<viewers.length;i++){if(viewers[i].toLowerCase()==="html5mobile"){if(this.options.documentType&&this.options.documentType!=="xod")continue;allowHTML5=true;if(me._testHTML5()){if(this.options.mobileRedirect){me.selectedType="html5Mobile";var newLocation=this.options.html5MobilePath+
this._getHTML5OptionsURL();window.location=newLocation;return}if(this.options.xdomainProxyUrl||me.isSameOrigin(decodeURIComponent(me.options.initialDoc))||me._testCORS()){me.selectedType="html5Mobile";break}}}if(viewers[i].toLowerCase()==="html5"){allowHTML5=true;if(me._testHTML5()){var sameOrigin=me.isSameOrigin(decodeURIComponent(me.options.initialDoc));if(this.options.xdomainProxyUrl||sameOrigin||me._testCORS()){me.selectedType="html5";break}}}}if(me.selectedType==="html5")me._createHTML5();else if(me.selectedType===
"html5Mobile")me._createHTML5Mobile();else if(allowHTML5){var supportError="Please use an HTML5 compatible browser";$(me.element).append('<div id="webviewer-browser-unsupported">'+supportError+".</div>")}},_viewerLoaded:function(iframe){var me=this;me._trigger("ready");var viewerWindow=iframe.contentWindow;var outerWindow=window;outerWindow.CoreControls=outerWindow.CoreControls||{};outerWindow.CoreControls.DisplayModes=viewerWindow.CoreControls.DisplayModes;if(typeof me.options.encryption!=="undefined"){var doc=
decodeURIComponent(me.options.initialDoc);var loadOptions={decrypt:viewerWindow.CoreControls.Encryption.decrypt,decryptOptions:me.options.encryption};viewerWindow.ControlUtils.byteRangeCheck(function(status){if(status===200)loadOptions.streaming=true;me.loadDocument(doc,loadOptions)},function(){loadOptions.streaming=true;me.loadDocument(doc,loadOptions)})}if(me.instance.docViewer.getDocument()===null)viewerWindow.$(iframe.contentDocument).bind("documentLoaded",function(event){me._trigger(event.type)});
else me._trigger("documentLoaded");viewerWindow.$(iframe.contentDocument).bind("displayModeChanged layoutModeChanged zoomChanged pageChanged fitModeChanged toolModeChanged printProgressChanged error",function(event,data1,data2){me._trigger(event.type,null,data1,data2)})},_isPDFExtension:function(extension){var result=false;if(extension){var pdfExtensions=[".pdf",".png",".jpg","jpeg"];pdfExtensions.forEach(function(ext){if(extension===ext)result=true})}return result},_isOfficeExtension:function(extension){var result=
false;if(extension){var officeExtensions=[".docx",".doc",".pptx"];officeExtensions.forEach(function(ext){if(extension===ext)result=true})}return result},_getHTML5OptionsURL:function(){if(this.selectedType==="html5"){if(this.options.html5Options)$.extend(this.options,this.options.html5Options)}else if(this.selectedType==="html5Mobile")if(this.options.html5MobileOptions)$.extend(this.options,this.options.html5MobileOptions);var options=this.options;var url="";var me=this;function acceptBackendUrl(officeEnabled,
pdfEnabled){if(officeEnabled)url+=options.backendType?"&office="+options.backendType:"&office=auto";if(pdfEnabled)url+=options.backendType?"&pdf="+options.backendType:"&pdf=auto"}if(typeof options.initialDoc!=="undefined")url+="#d="+options.initialDoc;var officeEnabled=false,pdfEnabled=false;var periodPosition;var extension=typeof options.initialDoc!=="undefined"&&(periodPosition=options.initialDoc.lastIndexOf("."))?options.initialDoc.slice(periodPosition).toLowerCase():null;if(options.documentType===
"pdf"||options.documentType==="all"||typeof options.documentType==="undefined"&&this._isPDFExtension(extension))pdfEnabled=true;if(options.documentType==="office"||options.documentType==="all"||typeof options.documentType==="undefined"&&this._isOfficeExtension(extension))officeEnabled=true;if(typeof options.backendType==="undefined")options.backendType=options.pdfBackend;acceptBackendUrl(officeEnabled,pdfEnabled);if(options.streaming)url+="&streaming=true";if(options.externalPath){var path=this._correctRelativePath(options.externalPath);
path=encodeURIComponent(path);url+="&p="+path}if(options.encryption)url+="&auto_load=false";if(options.enableAnnotations)url+="&a=1";if(options.serverUrl){var serverUrl=this._correctRelativePath(options.serverUrl);serverUrl=encodeURIComponent(serverUrl);url+="&server_url="+serverUrl}if(options.documentId)url+="&did="+options.documentId;if(options.config){var config=this._correctRelativePath(options.config);config=encodeURIComponent(config);url+="&config="+config}if(options.enableOfflineMode)url+=
"&offline=1";if(options.startOffline)url+="&startOffline=1";if(options.enableReadOnlyMode)url+="&readonly=1";if(options.hideAnnotationPanel)url+="&hideAnnotationPanel=1";if(typeof options.annotationUser!=="undefined")url+="&user="+options.annotationUser;if(typeof options.annotationAdmin!=="undefined")url+="&admin="+(options.annotationAdmin?1:0);if(typeof options.custom!=="undefined")url+="&custom="+encodeURIComponent(options.custom);if(typeof options.showLocalFilePicker!=="undefined")url+="&filepicker="+
(options.showLocalFilePicker?1:0);if(typeof options.preloadPDFWorker!=="undefined")url+="&preloadWorker="+(options.preloadPDFWorker?1:0);if(typeof options.pdfnet!=="undefined")url+="&pdfnet="+(options.pdfnet?1:0);if(typeof options.showToolbarControl!=="undefined")url+="&toolbar="+(options.showToolbarControl?"true":"false");if(typeof options.showPageHistoryButtons!=="undefined")url+="&pageHistory="+(options.showPageHistoryButtons?1:0);if(typeof options.xdomainProxyUrl!=="undefined"){var urls;if(typeof options.xdomainProxyUrl===
"string")urls={url:options.xdomainProxyUrl};else urls=options.xdomainProxyUrl;url+="&xdomain_urls="+encodeURIComponent(JSON.stringify(urls))}if(options.azureWorkaround)url+="&azureWorkaround=1";if(!options.useDownloader)url+="&useDownloader=0";if(options.subzero)url+="&subzero=1";if(typeof options.workerTransportPromise!=="undefined")url+="&useSharedWorker="+(options.workerTransportPromise?"true":"false");if(url.length>0&&url[0]==="&")url="#"+url.slice(1);return url},_createHTML5:function(){var me=
this;var iframeSource=this.options.html5Path+this._getHTML5OptionsURL();var $rcFrame=$(document.createElement("iframe"));$rcFrame.attr({id:this.rcId,src:iframeSource,frameborder:0,width:"100%",height:"100%",allowFullScreen:true,webkitallowfullscreen:true,mozallowfullscreen:true});if(this.options.backgroundColor)$rcFrame.attr("data-bgcolor",this.options.backgroundColor);$rcFrame.on("load",function(){me.instance=this.contentWindow.readerControl;var iframe=this;if(typeof me.instance==="undefined")this.contentWindow.$(this.contentDocument).bind("viewerLoaded",
